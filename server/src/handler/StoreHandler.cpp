...
void StoreHandler::handleStoreListRequest(std::shared_ptr<ClientSession> session, const nlohmann::json &requestJson)
{
    try
    {
        ...
        session->sendPacket(RES_STORE_LIST, resJson.dump());
    ...
    catch (const std::exception &e)
    {
        ...
        session->sendPacket(RES_STORE_LIST, nlohmann::json(errorDto).dump());
    }
}
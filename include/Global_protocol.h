...
enum class CmdID : uint16_t
{
    ...
    RES_REQUEST_NO = 5001,        // 1대1 문의 요청 거절
    NOTIFY_CHAT_EXIT = 5002,      // 채팅 종료 알림 (관리자 또는 사장님이 채팅 종료 시 서버에서 브로드캐스팅)
    ...
    NOTIFY_NEW_CHAT_REQUEST = 9030, // 서버 -> 관리자 전원: "사장님/고객님이 1:1 채팅을 요청했습니다!"
                                    // 2. 방 개설 (수락) 알림
    NOTIFY_CHAT_ROOM_OPENED = 9031, // 서버 -> 사장님/고객: "관리자가 수락하여 채팅방이 열렸습니다!"
                                    // 3. 메시지 수신 알림 (수신자에 따라 완벽 분리)
    NOTIFY_MSG_TO_USER = 9032,      // 서버 -> 사장님/고객: 관리자가 보낸 메시지 수신
    NOTIFY_MSG_TO_ADMIN = 9033,     // 서버 -> 관리자: 사장님/고객이 보낸 메시지 수신
    NOTIFY_ADMIN_CHAT_REQ = 9040    // 서버 -> 관리자: 고객의 1:1 문의 요청 알림
};

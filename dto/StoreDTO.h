...
struct StoreDTO
{
    ...
    int status;                  // 영업 상태 (0: 영업 중, 1: 휴업, 2: 폐업)
    nlohmann::json deliveryFees; // 배달비 정보 (예: {"baseFee": 3000, "perKm": 500})
    int cookTime;                // 조리 시간 (분 단위, 예: 30)
    std::string imageUrl;        // 가게 대표 이미지 URL
...
{
    int categoryId;
    std::string brandName; // 🚀 [추가] 프론트가 "GS25"라고 알려줄 수 있게 추가!

    // 🚨 중요: WITH_DEFAULT로 바꿔야 합니다!
    // 일반 '한식' 탭을 누를 때는 프론트가 brandName을 안 보낼 수도 있는데,
    // WITH_DEFAULT를 쓰면 에러 없이 알아서 빈 문자열("")로 채워줍니다.
    NLOHMANN_DEFINE_TYPE_INTRUSIVE_WITH_DEFAULT(StoreListReqDTO, categoryId, brandName)
};

...
struct ResCheckoutInfoDTO // 서버 -> 클라이언트: "여기 주문 상세 영수증 정보입니다!" 응답 DTO
{
    ...
    int appliedDeliveryFee;    // 고객이 선택한 배달 방식에 따른 배달비 (예: 0원 또는 3500원)
    // --- 매장 정보 (STORES 테이블) ---
    ...
    NLOHMANN_DEFINE_TYPE_INTRUSIVE_WITH_DEFAULT(ResCheckoutInfoDTO,
                                                status, customerGrade, cardNumber, accountNumber, userPoint, userAddress, appliedDeliveryFee,
...
struct ResOrderHistoryDTO // 서버 -> 클라이언트: "여기 주문 내역 리스트입니다!" 응답 DTO
{
    ...
    NLOHMANN_DEFINE_TYPE_INTRUSIVE_WITH_DEFAULT(ResOrderHistoryDTO, status, historyList)
};
...
struct ReqOrderHistoryDTO // 클라이언트 -> 서버: "내 과거 주문 내역 다 주세요!" 요청 DTO
{
    ...
    NLOHMANN_DEFINE_TYPE_INTRUSIVE_WITH_DEFAULT(ReqOrderHistoryDTO, userId) // JSON <-> Struct 자동 변환 매크로
};
